...
<head>
  ...
  <style>
    ...
    * {
    ...
    .btn-inv-copy { background: #334155; color: #fff; border: 1px solid var(--border); width: 100%; }
...
<body>
    ...
    <div class="my-world-container">
      ...
      <div class="bubbles-grid" id="bubblesGrid">
        ...
        <div class="bubble" onclick="openDrawer('invoices')">
          ...
          <div class="bubble-desc">Staged & ready to bill</div>
        </div>
  ...
  <script>
    ...
    function processInput(text) {
      ...
      var isInvoice = (lower.indexOf('invoice') !== -1 || lower.indexOf('bill') !== -1 || lower.indexOf('bag') !== -1 || lower.indexOf('hose') !== -1 || lower.indexOf('tune') !== -1 || lower.indexOf('$') !== -1);

      if (isInvoice) {
        let items = [];
        let total = 0;

        if (lower.indexOf('bag') !== -1) { items.push("• 9 Pk. Clean Obsessed HEPA Bags: $20.00"); total += 20.00; }
        if (lower.indexOf('hose') !== -1) { items.push("• Vacuum Hose Assembly: $35.00"); total += 35.00; }
        if (lower.indexOf('tune') !== -1 || lower.indexOf('labor') !== -1) { items.push("• Equipment Labor & Maintenance: $40.00"); total += 40.00; }
        ...
        if (items.length === 0 && match) {
          ...
          items.push("• Staged Item: " + text);
          total = 40.00;
        }

        var totalStr = "$" + total.toFixed(2);
        stageInvoice(text, totalStr, items);
        addMsg('💵 Staged Invoice for ' + totalStr + '. Copy details or view in My World!', 'fresh');
      } else if (lower.indexOf('grocery') !== -1 || lower.indexOf('milk') !== -1 || lower.indexOf('home') !== -1) {
    ...
    function stageInvoice(text, amountStr, itemsList) {
      ...
      if (amt) amt.innerText = amountStr;
      if (details) details.innerHTML = '<strong>Line Items:</strong><br>' + (itemsList ? itemsList.join('<br>') : text);
      if (card) card.style.display = 'flex';

      state.inv.unshift({ title: text, amount: amountStr, date: new Date().toLocaleDateString() });
      updateCounts();
    ...
    function updateCounts() {
      ...
      for (var i = 0; i < state.inv.length; i++) {
        var num = parseFloat(state.inv[i].amount.replace('$', '')) || 0;
        totalInvAmount += num;
      }

      if (cb) cb.innerText = state.biz.length + ' Jobs';
      if (cp) cp.innerText = state.pers.length + ' Items';
      if (ci) ci.innerText = state.inv.length > 0 ? ('$' + totalInvAmount.toFixed(2) + ' (' + state.inv.length + ')') : '$0 Due';
    }
    ...
    async function loadBackendMyWorld() {
      try {
        var res = await fetch('/.netlify/functions/myworld');
        var data = await res.json();
        if (data && data.groups && Array.isArray(data.groups)) {
          // Sync backend myworld groups if present
        }
      } catch(e){}
    }
    ...
    function openDrawer(cat) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (cat === 'invoices') {
          card.innerHTML = '<div class="cascade-card-top"><strong>' + items[i].title + '</strong><span style="color:#10b981;font-weight:800;">' + items[i].amount + '</span></div>';
        } else {
